function [params, BIC] = pdb_Perceptual_ShiftModel(behdata)

% Bharath Talluri & Anne Urai
% code accompanying the post-decision bias paper. This code calculates
% model based weights for the shift model.
...
for sj = subjects
    ...
    starting_pt = [datasample(1:5:25, 1) datasample(0.05:0.05:1, 1) datasample(0.05:0.05:1, 1) datasample(0:0.5:5, 1)];
...
[subj_params,subj_NlogL] = fminsearchbnd(@(individualparams) pdb_model(individualparams),individualparams,[0,-1000,-1000, -1000],[80,1000,1000, 1000],options);
end
...
for i = 1:num_trls
    ...
    Y2cc = normpdf(X, (X2(i) + psycho_bias)*individualparams(3) + ReadlDecision(i).*individualparams(4), psycho_noise*abs(individualparams(3)));
