function [setup, dots, fix, results, sound, stim] = configuration_training(window, audio, setup)
...
perm        = unique(nchoosek(repmat([-20 -10  10 20], 1,5), 2), 'rows');
...
setup.trialrep          = 1; % total nr of trials per permutation - within choice only
...
% for i = 1:length(setup.randomize)/setup.percentagechoiceonlytrials, %go over the choice only trials
%     assert(isequal(stim.one(find(setup.randomize==i), :, :, :), ...
%         stim.one(find(setup.randomize==(i+length(perm))), :, :, :), ...
%         stim.one(find(setup.randomize==(i+2*length(perm))), :, :, :)), ...
%         'Could not assert identical dot coordinates in the choice and no choice trials');
%     % assert is a great command to check important parts of your code, and
%     % outputs an error message if the condition is not true
% end
